...
def main():
    ...
    while True:
        cmd = raw_input('please entry your command [h for help]:')
        if cmd == 'h' or cmd == 'H':
            print '''
[list,l,ll] - [list folder]
[reset,r] - [reset index]
[index, i] - [index]
    args: folder1 folder2
    example: i /tmp /home/yihan
[search s] - [search]
    args: regexp, file_regexp
    example: s 'samewold' *.php

'''
        _cmd = cmd.split(' ')[0]
        _args = cmd.split(' ')[1:]

        if _cmd.upper() in ('LIST', 'L', 'LL'):
            cs.list()
        if _cmd.upper() in ('RESET', 'R'):
            cs.reset()
        if _cmd.upper() in ('I', 'INDEX'):
            cs.index(_args)
        if _cmd.upper() in ('S', 'SEARCH'):
            cs.search(''.join(_args[:1]), ''.join(_args[1:]))
        if _cmd.upper() in ('N','n'):
            cs.add_file()
